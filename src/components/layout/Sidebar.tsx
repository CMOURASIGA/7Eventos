    ...
    <>
      ...
      >
        <div className="relative border-b border-white/15 px-4 pb-6 pt-4">
          ...
          <button type="button" onClick={onCloseMobile} className="sidebar-close absolute right-3 top-3 md:hidden" aria-label="Fechar menu">×</button>
          <div className="mt-6 px-2">
            <div className="flex items-center gap-2">
              <p className="sidebar-product-name">7Eventos</p>
              {isDemo && <span className="rounded-md border border-amber-300/50 bg-amber-300/15 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide text-amber-200">Demo</span>}
            </div>
            ...
            <div key={section}>
              ...
              <div className="flex flex-col gap-1">
                {items.filter((item) => item.section === section).map((item) => {
                  ...
                  return <Link key={item.href} href={item.href} title={item.label} aria-current={active ? "page" : undefined} className={`sidebar-nav-link flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${active ? "sidebar-nav-link-active shadow-sm" : ""}`} onClick={onCloseMobile}><Icon /><span>{item.label}</span></Link>;
                })}
              </div>
            </div>
          ))}
