...
export const supabaseRepository: Repository = {
  companies: {
    ...
    async updateBranding(session, input) {
      ...
      if (!currentRow) throw new Error("Empresa não encontrada.");
      const current = mapCompany(currentRow);
      const row = unwrap<Row>(
        await db
          .from("companies")
          .update({
            configuracoes: { ...current.configuracoes, ...input },
      ...
      await recordAudit(session, {
        ...
        descricao: "Identidade visual da empresa atualizada.",
      });
  ...
  audit: {
    ...
    async countInteractions(session, filters) {
      ...
      if (filters.onlyBillable) query = query.eq("metadados->>consomeCota", "true");
      const { data, error } = await query;
      if (error) throw new Error(error.message);
      const rows = data ?? [];
      return {
